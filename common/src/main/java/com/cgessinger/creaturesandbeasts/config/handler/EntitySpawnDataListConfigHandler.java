...
public class EntitySpawnDataListConfigHandler implements IConfigElementHandler<List<EntitySpawnData>, List<Config>> {
    ...
    @Override
    public List<EntitySpawnData> deserialize(IConfigElement<List<EntitySpawnData>> iConfigElement, List<Config> configs) {
        if (configs != null) {
            return configs.stream().map(EntitySpawnData::fromConfig).collect(ImmutableList.toImmutableList());
        }
        return null;
    }
