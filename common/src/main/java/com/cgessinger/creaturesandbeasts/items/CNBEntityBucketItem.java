...
public class CNBEntityBucketItem extends BucketItem {
    ...
    @Override
    public void checkExtraContent(@Nullable LivingEntity entity, Level worldIn, ItemStack stack, BlockPos pos) {
    ...
    @Override
    protected void playEmptySound(@Nullable LivingEntity entity, LevelAccessor worldIn, BlockPos pos) {
        worldIn.playSound(entity, pos, this.emptyingSoundSupplier.get(), SoundSource.NEUTRAL, 1.0F, 1.0F);
    }

    private void placeEntity(ServerLevel worldIn, ItemStack stack, BlockPos pos) {
        Entity entity = this.entityTypeSupplier.get().spawn(worldIn, stack, null, pos, EntitySpawnReason.BUCKET, true, true);
    ...
    public InteractionResult use(Level worldIn, Player playerIn, InteractionHand handIn) {
        InteractionResult actionResult = super.use(worldIn, playerIn, handIn);
